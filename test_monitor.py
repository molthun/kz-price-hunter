...
class TestForteMarketScraper(unittest.TestCase):
    ...
    def test_registry_contains_iteka(self):
        from web.server import SHOP_REGISTRY
        from config import SHOP_KEYS, ITEKA_CATEGORIES
        self.assertIn("iteka", SHOP_REGISTRY)
        scraper_cls, cats, name = SHOP_REGISTRY["iteka"]
        self.assertEqual(name, "i-Teka")
        self.assertEqual(cats, ITEKA_CATEGORIES)
        self.assertEqual(SHOP_KEYS.get("iteka"), "i-Teka")
        for c in ITEKA_CATEGORIES:
            self.assertIn("master", c)
            self.assertIn("url", c)
            self.assertTrue(c["master"] in config.MASTER_CATEGORIES)
    ...
    def test_registry_contains_planeta(self):
        from web.server import SHOP_REGISTRY
        from config import SHOP_KEYS, PLANETA_CATEGORIES
        self.assertIn("planeta", SHOP_REGISTRY)
        scraper_cls, cats, name = SHOP_REGISTRY["planeta"]
        self.assertEqual(name, "Планета Электроники")
        self.assertEqual(cats, PLANETA_CATEGORIES)
        self.assertEqual(SHOP_KEYS.get("planeta"), "Планета Электроники")
        for c in PLANETA_CATEGORIES:
            self.assertIn("master", c)
            self.assertIn("url", c)
            self.assertTrue(c["master"] in config.MASTER_CATEGORIES)

    def test_registry_contains_kimex(self):
        from web.server import SHOP_REGISTRY
        from config import SHOP_KEYS, KIMEX_CATEGORIES
        self.assertIn("kimex", SHOP_REGISTRY)
        scraper_cls, cats, name = SHOP_REGISTRY["kimex"]
        self.assertEqual(name, "KIMEX")
        self.assertEqual(cats, KIMEX_CATEGORIES)
        self.assertEqual(SHOP_KEYS.get("kimex"), "KIMEX")
        for c in KIMEX_CATEGORIES:
            self.assertIn("master", c)
            self.assertIn("url", c)
            self.assertTrue(c["master"] in config.MASTER_CATEGORIES)
    ...
    def test_registry_contains_french_house(self):
        from web.server import SHOP_REGISTRY
        from config import SHOP_KEYS, FRENCH_HOUSE_CATEGORIES
        self.assertIn("french_house", SHOP_REGISTRY)
        scraper_cls, cats, name = SHOP_REGISTRY["french_house"]
        self.assertEqual(name, "Французский Дом")
        self.assertEqual(cats, FRENCH_HOUSE_CATEGORIES)
        self.assertEqual(SHOP_KEYS.get("french_house"), "Французский Дом")
        for c in FRENCH_HOUSE_CATEGORIES:
            self.assertIn("master", c)
            self.assertIn("url", c)
            self.assertTrue(c["master"] in config.MASTER_CATEGORIES)
