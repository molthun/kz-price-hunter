...
class SellerIdentityRepository(BaseRepository):
    ...
    def add_to_review_queue(
        ...
        now = utc_now_iso()
        # Идентификатор строится из самой пары, без времени: та же спорная пара при повторной
        # оценке обновляет существующую запись, а не создаёт новую. Иначе при периодическом
        # discovery очередь наполнялась бы копиями одного и того же вопроса.
        queue_id = f"srq_{hashlib.sha256(f'{candidate_seller_id}:{matched_seller_id}'.encode()).hexdigest()[:12]}"
        sql = """
    ...
    # Alias for discovery pipeline compatibility
    enqueue_review = add_to_review_queue

    def get_review_queue(self, status: Optional[str] = "PENDING", limit: int = 50) -> List[Dict[str, Any]]:
        """Returns review queue items filtered by status (defaults to PENDING)."""
        sql = "SELECT * FROM seller_review_queue WHERE 1=1"
        params: list = []
        if status:
            sql += " AND status = ?"
    ...
    def get_pending_reviews(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        return self.get_review_queue(status="PENDING", limit=limit)

    RESOLVED_DECISIONS = ("APPROVED", "REJECTED")

    def resolve_review(self, queue_id: str, decision: str, actor: Optional[str] = None) -> bool:
        """Закрывает запись очереди модерации принятым решением.

        Без этого метода очередь была журналом: записи могли только накапливаться, и решение
        REVIEW никуда не вело. Закрытая запись остаётся в таблице — история решений по спорным
        парам нужна, чтобы один и тот же вопрос не поднимался заново.
        """
        if decision not in self.RESOLVED_DECISIONS:
            raise ValueError(f"Недопустимое решение модерации: {decision}")
        row = self.fetchone("SELECT status FROM seller_review_queue WHERE id = ?", (queue_id,))
        if not row:
            return False
        if row["status"] != "PENDING":
            return False
        details = {"resolved_by": actor} if actor else {}
        self.execute(
            """UPDATE seller_review_queue
               SET status = ?, updated_at = ?,
                   details_json = json_patch(COALESCE(details_json, '{}'), ?)
               WHERE id = ?""",
            (decision, utc_now_iso(), json.dumps(details, ensure_ascii=False), queue_id),
        )
        return True

    def shared_identity_values(self, min_sellers: int = 2) -> set:
        """Пары (тип, значение), которые встречаются больше чем у min_sellers продавцов.

        Общий колл-центр, общий хостинг или адрес франшизы по форме не отличить от настоящего
        отпечатка — их выдаёт только частота. Матчер принимает это множество и не считает такие
        значения сильными ключами.
        """
        rows = self.fetchall(
        ...
        return {(r["identity_type"], r["identity_value"]) for r in rows}

    def match_candidate(self, candidate: Dict[str, Any], seller) -> Any:
        """Сопоставляет кандидата с продавцом, подставляя частотный признак из базы.

        Матчер умеет не доверять значениям, которые делят многие продавцы, но сам их не знает:
        частоту видно только в базе. Без этой обвязки защита оставалась бы на бумаге — именно так
        реселлер, ссылающийся на сайт бренда, склеивался бы с официальным магазином.
        """
        from domain.seller_matcher import SellerMatcher

        return SellerMatcher.match_candidate_against_seller(
            candidate,
            seller,
            existing_identities=[i.to_dict() for i in self.get_identities_by_seller(seller.id)],
            shared_values=self.shared_identity_values(),
        )
