...
1. Создайте бота у [@BotFather](https://t.me/BotFather) и укажите домен сайта: `/setdomain` → `shop.molthun.ru`.
...
Виджет входа Telegram работает только на домене бота, поэтому локально войти можно кнопкой «Вход разработчика»:

```bash
ALLOW_DEV_LOGIN=1 ADMIN_TELEGRAM_IDS=1 ./venv/bin/python3 gui.py
```
...
| `ALLOW_DEV_LOGIN` | — | `1` — кнопка «Вход разработчика» для локальной разработки (работает только с localhost / локальной сети без reverse proxy). **Не включайте на проде** |
