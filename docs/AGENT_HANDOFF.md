...
- Дата: 2026-09-20 12:56, Asia/Almaty.
- Последнее действие: Claude закрыл остаток E01 — `type_totals` считается после общих фильтров списка (магазин,
  категория, поиск) и до выбора вкладки и страницы. Ветка `dev/p05-settings`.
- P03/P05 READY по коду; P04 — REVIEW, нужен точечный аудит остатка E01. Пачка не выпущена.
- Правило владельца: **без аудита ничего не пушим**. Push, выпуск и изменения прода не выполнялись.
- Тесты: Python **432 OK**, frontend **7/7 PASS**. Новые регрессии падают на коде до исправления. Плитка по городу
  не изменилась (сверено на копии рабочей базы; рабочая БД не тронута).
- Пишущих исполнителей нет: Claude закончил. Следующий — Codex: точечный аудит остатка E01.
- Команда для Codex: «Прочитай AGENTS.md, docs/DEVELOPMENT_PLAN.md и docs/AGENT_HANDOFF.md. Проведи точечный аудит
  остатка E01 P04 на ветке dev/p05-settings, не меняя код (diff a467b18..HEAD, раздел «Остаток E01» в
  docs/P04_UI_DATA_ANALYSIS.md): разбивка совпадает с итогом вкладки при магазине, категории, поиске и их
  комбинациях; пагинация и сортировка счётчики не меняют; городская плитка не изменилась. Запиши выводы в
...
| P04 | IN_PROGRESS | Claude | Codex: E02 закрыто; E01 — фильтры разбивки API | Не выпущен |
...
Статус: REVIEW — нужен точечный аудит остатка E01 (команда — в «Текущей контрольной точке»).
