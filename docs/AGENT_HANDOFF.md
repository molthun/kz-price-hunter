...
- Дата: 2026-09-20 13:02, Asia/Almaty.
- Последнее действие: Claude подготовил выпуск **5.10.0** (P03+P04+P05) по поручению владельца «Подготовить выпуск».
  Ветка `dev/p05-settings` слита в локальный main (merge `66ab926`), версия поднята, CHANGELOG/ROADMAP/README обновлены,
  релизный коммит `c470aec`, тег `v5.10.0` создан **локально**.
- **Push не выполнялся.** Локальный main впереди origin/main на: docs-коммит v5.9.0 (0426523), пачку P03–P05, релиз.
  Отправка в GitHub = автодеплой на shop.molthun.ru (~5 мин) — только по отдельному слову владельца «выпускай».
- Релизные проверки на копии рабочей базы (копия в scratchpad; рабочая `prices.db` не тронута):
  - Python **432 OK**, frontend **7/7 PASS**.
  - Старт 5.10.0 на копии: `/api/version` → 5.10.0, `schema_version` остался **5**, `/monitoring` и `/static/js/format.js`
    отвечают 200, `/api/monitoring/*` гостю недоступен; счётчики вкладок витрины совпадают с разбивкой при фильтрах
    (Астана: 10 980 / супер 336 / арбитраж 78; Sulpak + «Samsung»: 70 / 6 / 3).
  - **Откат проверен**: код v5.9.0 запущен на той же обновлённой базе — стартует, отвечает 5.9.0, главная 200,
    ошибок в логе нет. Восстановление БД для отката не требуется.
- Не проверено: Docker-образ, реальные обходы магазинов, AI и Telegram, прод (на нём сейчас v5.9.0).
- Пишущих исполнителей нет: Claude закончил. Следующий шаг — по решению владельца: «выпускай» (push main + тег) либо
  следующий этап P06.
...
| P03 | READY (в выпуске 5.10.0, локально) | Claude | Codex: D01/D02 закрыты | Не выпущен: тег v5.10.0 создан локально, push не выполнялся |
| P04 | READY (аудит кода) | Claude | Codex: E01/E02 закрыты | Не выпущен |
| P05 | READY (в выпуске 5.10.0, локально) | Claude | Codex: замечаний нет | Не выпущен: тег v5.10.0 создан локально, push не выполнялся |
