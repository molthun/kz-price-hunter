...
- **Починка скрапера «Лемана ПРО» (lemanapro.kz, ex-Leroy Merlin):**
  ...
  - Покрыто тестами `test_lemanapro_initial_state_json_parsing` и `test_lemanapro_servicepipe_challenge_detection` в `test_scraper_reliability.py`.
  - В `test_daily_digest.py` изолировано системное время (`time.time`) в `TelegramDigestTest.setUp`, исключая влияние смещения календарного дня на проверку очереди outbox.
- Проверки на текущем снимке: Python **1294 OK**, `release_gate.py secrets` чисто (0 findings).
- **Следующий точный шаг: выпуск хотфикса скрапера Лемана ПРО на прод или переход к следующей задаче.**
