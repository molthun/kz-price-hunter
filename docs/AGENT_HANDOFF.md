...
- Дата: 2026-09-24, Asia/Almaty. Исполнитель разработки — **Antigravity**. На проде **v5.15.0**.
- **Настройка моделей в режиме «Авто» (задача владельца):**
  - Google Gemini: стандартная модель переведена с недоступной `gemini-2.5-flash` (вызывавшей HTTP 404) на актуальную, быструю и экономичную `gemini-3.5-flash-lite` (отклик ~890 мс, тариф $0.30 / $2.50 за 1 млн токенов).
  - OpenAI: подтверждена оптимальная `gpt-4o-mini` (отклик ~1120 мс, тариф $0.15 / $0.60 за 1 млн токенов).
  - В `ai_service.py` реализован автоматический каскадный резерв (fallback) в режиме «Авто»:
    - Для Gemini при 404/429/500/502/503/504 пробуются `gemini-3.5-flash-lite` → `gemini-3.1-flash-lite` → `gemini-3.5-flash` → `gemini-flash-latest`.
    - Для OpenAI пробуются `gpt-4o-mini` → `gpt-4.1-mini`.
    - В ручном режиме выбранная пользователем модель не подменяется молча.
  - Обновлены подписи в веб-интерфейсе (`web/templates/index.html`) с указанием конкретных моделей и их качеств.
- Пробный шаг адаптивного планировщика идёт на `alser` (профиль normal). Сводка в Telegram и
  AI-сопоставление выключены.
- Выпуски после пачки: 5.13.1 … 5.13.4, 5.14.0 (список моделей от провайдера + починка потери цен),
  5.14.1 (карточка расходов), 5.14.2 (выбранная модель применяется), 5.15.0 (подсказка цен с
  подтверждением). Все собраны шлюзом P16 полностью.
- **Доработка скрапера DNS (по пункту 1 предложений Кодекса):**
  - В `scrapers/dns.py` добавлены флаги маскировки автоматизации Chromium (`--disable-blink-features=AutomationControlled`, скрытие `navigator.webdriver = false`).
  - Реализован перехват сетевых AJAX-ответов (`/ajax-state/product-buy/`, `/microdata/`, `/product/buy`) прямо из Playwright сессии — если DNS меняет верстку или отдает цены динамически через AJAX, они подтягиваются из перехваченного JSON как fallback.
  - Покрыто тестом `test_stealth_and_ajax_interception` в `test_scraper_reliability.py`.
- **Доработка UI и устойчивости отображения цен/карточек (по пункту 2):**
  - В `web/templates/index.html` исправлена логика карточек: нулевые зачёркнутые старые цены (`0 ₸`) больше не выводятся; экономия (`0 ₸`) скрывается, если выгода отсутствует; при сбое загрузки изображения верстка не схлопывается (замещается аккуратной плашкой).
- **Центр персональных наблюдений и Telegram (по пункту 3):**
  - В веб-интерфейсе добавлено модальное окно «Следить за ценой» (`modalWatchSetup`): позволяет пользователю при нажатии на колокольчик выбрать целевую цену (≤ X ₸), порог снижения (10%), отслеживание конкретного магазина или всех сетей по модели (`canonical_key`), а также однократный или повторяющийся режим.
  - В результатах поиска добавлена кнопка быстрой подписки на поисковый запрос/модель: «Следить за запросом».
  - В `notifier.py` сообщения по наблюдениям снабжены контекстными заголовками («🎯 Цена достигла вашей цели», «📉 Цена снизилась», «👑 Самая низкая цена за всё время») и инлайн-кнопкой мгновенной отписки прямо из Telegram (`🔕 Не следить`).
  - В `telegram_bot.py` реализована обработка интерактивных `callback_query` (отписка от наблюдения в 1 клик с сохранением изоляции приватного чата и проверкой блокировки).
  - Покрыто тестами в `test_bot_privacy.py`.
- **Редизайн пользовательских настроек и разделение контуров (по пункту 4):**
  - Пользовательский блок «Чувствительность поиска аномалий и скидок» переработан в понятный блок «Какие скидки показывать» с тремя человекопонятными пресетами: «Только крупные скидки» (от 50 000 ₸), «Обычный режим» (Рекомендуется, от 25 000 ₸), «Показывать больше» (от 2 000 ₸) и сворачиваемой секцией тонкой настройки («Дополнительные параметры (для опытных)»).
  - Внутренние детекторы («Пороги записи кандидатов») убраны из пользовательского интерфейса и вынесены в секцию администрирования как «Алгоритмы: Предварительный отбор изменений цены» с явным предупреждением о системном влиянии на пайплайн сбора.
  - «Состояние магазинов» отделено от настроек и оформлено как навигационный шлюз в Центр мониторинга (`/monitoring#shops`).
  - Настройка уровня Telegram переименована в «Какие скидки присылать в бот:» («Все подходящие скидки», «Только горячее и супер-скидки», «Только критические ошибки цен»).
  - Синхронизировано состояние радио-кнопок пресетов (`settingsPresetRadio`) при программной и ручной смене значений.
- **Развитие Центра мониторинга и Observability (по пункту 5 предложений Кодекса):**
  - Реализован принцип «норма молчит — аномалия заметна» на главном экране («Обзор», `/monitoring`): при нормальной работе всех источников отображается спокойный зелёный статус; при обнаружении деградации или проблемных магазинов (`degraded`, `offline`, `limited`, `empty`) наверху появляется яркий акцентный баннер со списком проблем, процентом потери каталога, ошибками HTTP и кнопкой мгновенного перехода к диагностике.
  - В модальном окне магазина (`shopModal` в `web/templates/monitoring.html`) добавлена подробная карточка HTTP: структурированные бейджи статус-кодов (200, 3xx, 401, 403, 429, 5xx), процент ошибок (`error_pct`), средняя задержка и P95, отказы по паузе/cooldown.
  - В карточке обходов магазина выводится базовый объём каталога (`baseline_items`), реальный процент покрытия (`coverage_pct`) с индикатором снижения (`↓ деградация`), а также автоматически вычисленный профиль адаптивного планировщика (`FRIENDLY`, `NORMAL`, `EXPENSIVE`, `DEGRADED`) и рекомендуемый интервал обхода.
  - В карточке раздела «Система» выведены точный Git коммит (SHA) и время непрерывной работы процесса (Uptime) в человекопонятном формате (`X д Y ч`).
- **Развитие аналитики поискового спроса и Search Success Rate (по пункту 6 предложений Кодекса):**
  - В `monitoring.py` (`search_section`) добавлен расчёт метрик верхнего уровня: `Search Success Rate`, доли слабых совпадений (`weak_rate`), нулевых исходов (`not_found_rate`) и процента ошибок.
  - В Центре мониторинга (`/monitoring`, вкладка «Поиск») выведен наглядный верхнеуровневый блок Search Success Rate с цветовой дифференциацией (`🟢 найдено`, `🟠 слабо`, `⚪ пусто`, `🔴 ошибки`).
  - В таблицах «🔥 Чаще всего ищут» и «⚠️ Что мы плохо находим» добавлены цветные индикаторы конверсии каждого запроса (`🟢 97%`, `🟠 45%`, `🔴 0%`).
  - В интерфейсе и документации зафиксирована прямая архитектурная связь неудовлетворённого спроса со scheduler (`database.py` + `scheduler_shadow.py`): нулевые/слабые поиски автоматически определяют релевантную категорию товаров через семантический анализатор и повышают приоритет её обхода в планировщике с коэффициентом x2.0 (`WEIGHT_UNMET_DEMAND`).
- **Интеллектуальный слой AI и помощник администратора (по пункту 7 предложений Кодекса):**
  - Подтверждено и зафиксировано трёхслойное разделение AI:
    1. Пользовательский AI: доступен исключительно авторизованным пользователям (`@require_login` на `/api/ai/consultant`, проверка сессии Telegram), с поиском по локальной базе цен и товаров, сравнением моделей и созданием наблюдений.
    2. Внутренний AI проекта: автоматическая классификация категорий товаров, объяснение инцидентов и диагностика сбоев магазинов (с изоляцией системного бюджета через `INTERNAL_BUDGET_SHARE = 0.7`).
    3. Каталожный AI (сопоставление): строгий приоритет детерминированных правил (`catalog_ai.py`), модель вызывается только при неполной фасовке (`may_ask`), порог уверенности 90%, результат кэшируется.
  - Маршрутизатор AI (`ai_router.py`): раздельные квоты и бюджеты для пользователей и внутренних нужд, защита от prompt injection через структурированный `untrusted_block` и каскадный fallback для моделей провайдеров.
  - В Центре мониторинга (`/monitoring`, Overview) обновлён блок «✨ Спросить KZ Price Hunter (Помощник администратора)»: добавлены быстрые кликабельные чипы вопросов («🔍 Почему DNS потерял товары?», «⚠️ Какие магазины сбоят?», «🎯 Что ищут и не находят?», «📅 Сводка состояния»), собирающие строго проверяемые факты и работающие исключительно в режиме Read-Only.
- **Сквозной аудит и доработки фундамента Observability, Data Quality и Event Journal (по синтез-пункту Кодекса):**
  - Проведён построчный аудит всех 16 аспектов надёжности (Massive, ScraperGuard, DataObservability):
    1. Качество данных как отдельный слой: подтверждено в `data_quality.py` (пороги брака, выпадения цен, фотографий, категорий).
    2. Freshness товара: в `modalProductDetails` (`web/templates/index.html`) добавлено отображение реального времени и статуса проверки цены («✓ Проверено N ч назад» / «⏳ От даты» / «⚠️ Устарела»).
    3. История и динамика источника: карточка магазина содержит раскладку по категориям, кодам HTTP, латентности и браку.
    4. Автоматическая базовая линия: вычисляется как динамическая медиана (`baseline_from_history`) по последним 5 чистым обходам.
    5. Контроль «тихих поломок» скраперов: брак > 30% или потеря объёма > 50% маркируют категорию как `degraded`.
    6. Приоритет данных (Watches): в `scheduler_shadow.py` подтверждён повышенный коэффициент веса `WEIGHT_WATCHES = 1.5` для товаров с активными наблюдениями.
    7. Контроль стоимости сбора: учёт `bytes_per_request`, `latency`, `error_pct` с переводом тяжёлых магазинов в профиль `EXPENSIVE`.
    8. Инциденты вместо моря логов: агрегация сотен 403/сбоев в единые инциденты с таймлайном и гипотезой.
    9. «Что изменилось за сутки» + AI-разбор: действует блок `daily_summary` и кнопка «Пересказать словами».
    10. Heartbeats: независимый мониторинг пульса `scheduler`, `telegram`, `ai_normalize`, `telemetry`, `backup`.
    11. Проверка бэкапов: регулярное тестовое открытие и верификация схемы SQLite в `backup_health.py`.
    12. Retention: политика очистки устаревших телеметрических событий собрана в одном месте.
    13. Feature flags: канареечные волны обходов и трёхпозиционный переключатель AI-матчинга (`off` / `shadow` / `on`).
    14. Единый Event Journal: в `web/templates/monitoring.html` лента событий обновлена до живого человекопонятного формата со структурированными иконками по компонентам (🟢, ⚠️, 🔴, 🔍, 🤖, 📉, 📱, 💾, 🌐), таймстемпами `HH:MM`, бейджами магазинов и выпадающими деталями.
- Проверки на текущем снимке: Python **1289 OK**, `release_gate.py secrets` чисто (0 findings).
- **Следующий точный шаг: переход к следующему пункту предложений Кодекса от владельца.**
...
| P07 | V1 DEPLOYED (v5.11.0); V2 DEPLOYED (v5.13.0) | Claude | Codex: G01/G02, M10 закрыты | v5.15.0 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть DEPLOYED (v5.13.0) | Claude | Codex: I01/I02, M11 закрыты | v5.15.0 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
...
| P11 | DEPLOYED (v5.13.0) | Claude | Codex: M01–M03 закрыты | v5.15.0 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
| P12 | DEPLOYED (v5.13.0) | Claude | Codex: M04 закрыт | v5.15.0 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
| P13 | DEPLOYED (v5.13.0) | Claude | Codex: M04–M06 закрыты | v5.15.0 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
