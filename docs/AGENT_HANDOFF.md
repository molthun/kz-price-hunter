...
- 18:55: P01 реализован и закоммичен на `dev/p00-baseline`, статус REVIEW, 299 тестов OK. Следующий шаг — независимый аудит P01.
...
Цель и критерии приёмки:
  ...
  - Добавить таблицы телеметрии и миграцию 6 (SCHEMA_VERSION = 6).
...
Начало и checkpoint: 2026-09-19 18:18 (Antigravity), подхват Claude 18:36, checkpoint 18:55 (Asia/Almaty)
Checkout / ветка / базовый HEAD / текущий HEAD:
  ...
  - Текущий HEAD: 12672ec (+ коммит P01, см. ниже)
...
Миграции / feature flags / данные для отката: миграция 6 аддитивная (только новые таблицы); init_db делает бэкап pre-v6.
  Откат: образ 5.7.1 на БД v6 не стартует (check_schema_compatible → SchemaTooNew, проверено по коду database.py:563).
  Откат после выката = восстановление бэкапа prices-pre-v6-*.db с потерей данных после выката; владельцу решить,
  приемлемо ли, или сделать миграцию 6 не повышающей schema_version (таблицы аддитивны). TELEMETRY_ENABLED=0 отключает сбор без отката.
...
Следующий точный шаг:
  - Независимый аудит P01 (Codex): проверить diff коммита, особенно scrapers/http.py (семантика лимитера) и стратегию
    отката (5.7.1 не стартует на schema_version 6). Затем решение владельца о P01-02 (остальные источники событий) и выпуске.
