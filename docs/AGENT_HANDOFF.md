...
- Дата: 2026-09-20 12:52, Asia/Almaty.
- Последнее действие: Claude исправил E01 и E02 по аудиту P04 (ветка `dev/p05-settings`, коммиты 204d2ab — отчёты
  Codex, и правка ниже). **P03 и P05 READY по аудиту кода; P04 — исправления готовы к повторному аудиту.**
- Отчёты аудита: [P03](P03_AUDIT_CODEX.md), [P04](P04_AUDIT_CODEX.md), [P05](P05_AUDIT_CODEX.md); итог исправлений —
  в конце [P04_UI_DATA_ANALYSIS.md](P04_UI_DATA_ANALYSIS.md).
- Правило владельца: **без аудита ничего не пушим**. Push и выпуск не выполнялись; на проде v5.9.0.
- Тесты после исправлений: Python **431 OK**, frontend **7/7 PASS** (Deno/Chrome, офлайн). Оба новых набора падают
  на коде до исправления. Дополнительно: счётчики сверены на копии рабочей базы (копия в scratchpad, рабочая БД не тронута).
- Пишущих исполнителей нет: Claude закончил. Следующий — Codex: повторный аудит P04.
- Команда для Codex: «Прочитай AGENTS.md, docs/DEVELOPMENT_PLAN.md и docs/AGENT_HANDOFF.md. Проведи повторный аудит
  P04 на ветке dev/p05-settings, не меняя код: исправления E01/E02 (diff 044602a..HEAD, раздел «Исправления по аудиту
  Codex» в docs/P04_UI_DATA_ANALYSIS.md). Проверь, что разбивка витрины и её вкладки совпадают при любых фильтрах и
  личных порогах, а алерты с устаревшей ценой не попадают ни в списки, ни в счётчики. Запиши выводы в
  docs/P04_AUDIT_CODEX.md, обнови карточку и реестр.»
- После приёмки P04 пачка P03+P04+P05 готова к выпуску 5.10.0 — только по отдельному поручению владельца.
...
| P04 | REVIEW (E01/E02 исправлены, ждёт повторного аудита) | Claude | Codex: E01/E02 — исправлено, аудит повторный | Не выпущен |
...
Статус: REVIEW — исправления готовы, нужен повторный аудит Codex (команда — в «Текущей контрольной точке»).
