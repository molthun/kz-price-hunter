...
- Дата: 2026-09-24, Asia/Almaty. Выпуск **v5.19.0** (автор — Antigravity, по прямому поручению владельца).
- **Что вошло в 5.19.0:**
  1) **Подключение 26-го магазина — сеть супермаркетов «Магнум» (magnum.kz):**
     - Реализован скрапер `MagnumScraper` на базе официального Strapi REST API (`magnum.kz:1337/api/new-product`).
     - Сбор актуальных цен (`final_price`), базовых цен (`start_price`), процента скидки (`discount`), условий акций и фотографий.
     - Настроены 15 товарных категорий: Бакалея, Чай/кофе, Молочные продукты, Мясо и птица, Фрукты и овощи, Гастрономия, Кондитерские изделия, Консервы, Напитки, Заморозка, Собственное производство (`grocery`), Бытовая химия, Средства гигиены, Детские товары (`household`) и полный сводный каталог акций (`actions`).
     - Реализован быстрый `search_live` для поиска скидок по запросам пользователей.
     - Поддержка городов Казахстана (Алматы, Астана, Шымкент, Караганда и др.).
  2) **Интеграция в веб-интерфейс и мониторинг:**
     - Магнум добавлен в иконки шапки (`index.html`), фильтры магазинов, админ-модалку настроек (`checkShopMagnum`) и белый список доменов описаний товаров (`product_details.py`).
  3) **Верификация и тесты:**
     - 1306 Python-тестов (контракт скрапера, валидация карточек, mock-тесты сбора, retry при 5xx, извлечение slug), 15 frontend-наборов, `release_gate.py secrets` (0 замечаний).
...
- **Следующий точный шаг: коммит, тег v5.19.0, пуш в main и проверка сборки CI.**
