...
- Дата: 2026-09-20, Asia/Almaty. Codex завершил независимый аудит пачки **P06–P09** на HEAD `a6c68cc`, ветка `dev/p09-catalog-quality`. Код проверен в неизменяемом экспорте `/tmp/kz-p09-audit.iozDg7`; дерево при приёме чистое.
- **P06: F01/F02/F04 закрыты; F03 (P1) открыт** — отказ live-источников всё ещё становится NOT_FOUND в пользовательской сводке. [Отчёт](P06_AUDIT_CODEX.md).
- **P07: G01/G02 (P1)** — одноразовые наблюдения отменяют собственную отправку; конкурентная оценка создаёт два события одного снижения. [Отчёт](P07_AUDIT_CODEX.md).
- **P08: H01/H02 (P1)** — расход user/internal общий; рабочие AI-пути обходят роутер, его запрет fallback и учёт. [Отчёт](P08_AUDIT_CODEX.md).
- **P09: I01/I02 (P1)** — новые ложные совпадения RAM/диагонали/назначения корма; совпавшая модель обходит неизвестную упаковку. Подтверждено через настоящую БД и отбор конкурентов. [Отчёт](P09_AUDIT_CODEX.md).
- Независимые проверки: **572 Python-теста OK, 36.440 с; frontend 9/9 PASS**. Все данные временные. Golden: 40 пар, TP=13/FP=0/TN=26/FN=1, precision=1.0/recall=0.9286; опасные случаи аудита отсутствуют в наборе и требуют расширения.
- **Следующий точный шаг: Claude исправляет F03, G01/G02, H01/H02, I01/I02**, добавляет описанные в отчётах сквозные регрессии и передаёт пачку на повторный независимый аудит. P06–P09 не READY. Новые этапы текущим аудитом не запускались.
- Изменены только `docs/P06_AUDIT_CODEX.md` … `docs/P09_AUDIT_CODEX.md` и этот реестр; без коммита/push, изменения кода и прода не выполнялись. Реальные AI/Telegram, Docker, нагрузка и прод не проверялись.
- Пишущих исполнителей аудита и фоновых команд аудита нет. Передача — Claude; другой агент автоматически не запускался. Правило владельца «без аудита ничего не пушим» сохраняется; выпуск этой пачки не рекомендован.
...
| P06 | IN_PROGRESS | Claude | Codex: F01/F02/F04 закрыты; F03 открыт | Не выпущен |
| P07 | IN_PROGRESS | Claude | Codex: G01/G02 требуют исправлений | Не выпущен |
| P08 | IN_PROGRESS | Claude | Codex: H01/H02 требуют исправлений | Не выпущен |
| P09 | IN_PROGRESS | Claude | Codex: I01/I02 требуют исправлений | Не выпущен |
