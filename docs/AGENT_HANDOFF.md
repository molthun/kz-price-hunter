...
- Дата: 2026-09-23 23:40, Asia/Almaty. Исполнитель — Claude.
- Последнее действие: **исправлены все 11 замечаний аудита Codex (M01–M11)** с регрессиями. Разбор по
  каждому замечанию и имена тестов — в [AUDIT_REQUEST_BATCH.md](AUDIT_REQUEST_BATCH.md), раздел
  «Исправления по аудиту». Отчёт аудита зафиксирован коммитом `99dd336`.
- **Push не выполнялся.** На проде v5.12.0, все выключатели новых возможностей в «выкл».
- Коммиты исправлений: `57999b7` (M04, M05, M06, M11), `e8b92b2` (M07, M08, M09),
  `bd8323d` (M01, M02, M03, M10). Ветка `dev/p07-watch-v2`, база `7375c91`.
- Главное по сути замечаний: состав пробного шага теперь фиксируется при включении и не переизбирается;
  метрики «после» считаются по часовым агрегатам от момента включения, поэтому прежние успехи не
  прячут провал canary; шаг нельзя выставить общим сохранением настроек; проверка чисел больше не
  прощает «12 ошибок»; счётчики событий не обрезаются до пяти; неизвестная стоимость AI не показывается
  нулём; копия для репетиции снимается через Backup API и откат подтверждается запуском прежнего кода;
  сканер секретов видит `.github`; непроверенная зависимость блокирует выкат; сводка уходит одним
  письмом; уверенность 0.8999 не дотягивает до порога 0.90.
- Проверки после исправлений: Python **1093 OK** (было 1047; добавлено 46 регрессионных тестов),
  frontend **14/14 PASS**, `release_gate.py secrets` — чисто. `schema_version` = 5.
- Не проведено (без изменений): прод, Docker, живой прогон GitHub Actions, реальные вызовы моделей и
  настоящая отправка в Telegram, замер AI-части P09 на живой модели, production canary.
- **Следующий точный шаг: повторная независимая приёмка Codex по обновлённому снимку.** После приёмки —
  решение владельца о выпуске (версия, CHANGELOG, тег, push) и отдельные решения о первых включениях.
