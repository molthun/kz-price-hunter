...
- Дата: 2026-09-20, Asia/Almaty. Codex завершил повторный аудит исправлений `aaa5fe6` на HEAD **e48ccb8**, ветка `dev/p09-catalog-quality`. Рабочее дерево при приёме чистое; проверка на экспорте `/tmp/kz-reaudit.6zTfMv`.
- **P06: F01–F04 закрыты**, READY по аудиту кода. Полный/частичный отказ, успешная пустая выдача, восстановление и единичный учёт проверены. [Отчёт](P06_AUDIT_CODEX.md).
- **P07 V1: G01/G02 закрыты**, READY по аудиту исправлений. Одноразовая доставка после временной ошибки работает; два конкурентных исполнителя дали одно событие без ошибок потоков. [Отчёт](P07_AUDIT_CODEX.md).
- **P08: IN_PROGRESS.** H02 (маршрутизация) закрыт; H01 (P1) остаётся в части атомарного ограничения квоты: два исполнителя расходуют 2 при лимите 1. Новый H03 (P2): запрещённый по квоте fallback считается сетевым вызовом и отказом провайдера. [Отчёт](P08_AUDIT_CODEX.md).
- **P09: I01/I02 закрыты**, детерминированная часть READY по аудиту исправлений; AI-часть ещё не реализована. [Отчёт](P09_AUDIT_CODEX.md).
- Уточнение golden v2: **53 пары, TP=16/FP=0/TN=35/FN=2, precision=1.0, recall=0.8889**. Прежние 0.94 из передачи не воспроизводятся; пороги 0.99/0.70 пройдены. Пропуски: iPad Air 11 и отдельная запись памяти MacBook Air.
- Независимые проверки: **638 Python-тестов OK, 41.996 с; frontend 9/9 PASS**. Данные временные, реальные AI/Telegram не вызывались. Docker, нагрузка и прод не проверены.
- **Следующий точный шаг: Claude исправляет H01/H03 в P08**, добавляет конкурентную проверку последней единицы квоты и проверку учёта заблокированного fallback; затем повторный аудит P08. Новые этапы не начинались. Выпуск общей пачки пока не рекомендован.
- Изменены только четыре отчёта `docs/P06_AUDIT_CODEX.md` … `docs/P09_AUDIT_CODEX.md` и этот реестр. Без коммита/push, код приложения, рабочая БД и прод не менялись. Фоновых команд аудита и пишущего аудитора нет; агент Claude автоматически не запускался.
...
| P04 | READY (аудит кода) | Claude | Codex: E01/E02 закрыты | Не выпущен |
...
| P06 | READY (аудит кода) | Claude | Codex: F01–F04 закрыты | Не выпущен |
| P07 | READY (V1, аудит исправлений) | Claude | Codex: G01/G02 закрыты | Не выпущен |
| P08 | IN_PROGRESS | Claude | Codex: H02 закрыт; исправить H01/H03 | Не выпущен |
| P09 | READY (детерминированная часть) | Claude | Codex: I01/I02 закрыты; AI-часть отдельно | Не выпущен |
