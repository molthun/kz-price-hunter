...
- Дата: 2026-09-23, завершение аудита Codex. Проверен HEAD `8c591fd49cbfe2afc956bbe54352b388b9ba71a1`, ветка `dev/p07-watch-v2`, база `7375c91`.
- **Пачка не принята: 11 подтверждённых замечаний M01–M11.** Отчёт с воспроизведениями и критериями: [BATCH_AUDIT_CODEX.md](BATCH_AUDIT_CODEX.md).
- P11: M01 — самовольная смена участников canary; M02 — смешение периодов до/после скрывает ухудшение; M03 — общие настройки обходят переходы и baseline.
- P12/P13: M04 — выдуманные числа 0–12 проходят валидатор. P13: M05 — счётчики событий обрезаны до пяти; M06 — неизвестная стоимость AI показывается нулём.
- P16: M07 — copy2 теряет WAL-транзакции при успешном результате gate; M08 — сканер секретов пропускает .github; M09 — непроверенные зависимости дают успех.
- P07 V2: M10 — 25 срабатываний сводки уходят двумя письмами. AI-часть P09: M11 — округление 0.8999 до 0.9 обходит порог принятия.
- Независимые проверки нового снимка: **1047 Python-тестов, 49.311 с, OK; 14 браузерных наборов успешно**. Только временные данные, AI/Telegram подменены. Все запущенные команды завершены.
- Во время начала аудита общий checkout изменился с P11 `e8fba6f` на новую пачку. Старый результат 882 теста относится только к тому снимку; новая пачка проверена отдельно. Авторская передача сохранена ниже.
- Docker, GitHub Actions, реальный аудит зависимостей, production canary и живой AI-замер не выполнены. Прод не менялся и в этой сессии не проверялся.
- Область правок Codex: только `docs/BATCH_AUDIT_CODEX.md` и этот реестр; без изменения приложения, коммита, push, merge или выпуска.
- **Следующий точный шаг: Claude исправляет M01–M11 с регрессиями, обновляет эту передачу и возвращает пачку на независимую приёмку.** Выпуск и первые включения остаются отдельными решениями владельца по заявке. Агент исправления автоматически не запускался.
...
| P07 | V1 DEPLOYED (v5.11.0); V2 IN_PROGRESS | Claude | Codex: G01/G02 закрыты (V1); M10 открыт (V2) | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть IN_PROGRESS | Claude | Codex: I01/I02 закрыты (правила); M11 открыт (AI) | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
| P11 | IN_PROGRESS | Claude | Codex: M01–M03 открыты | Не выпущен |
| P12 | IN_PROGRESS | Claude | Codex: M04 открыт | Не выпущен |
| P13 | IN_PROGRESS | Claude | Codex: M04–M06 открыты | Не выпущен |
...
| P16 | IN_PROGRESS | Claude | Codex: M07–M09 открыты | Не выпущен |
