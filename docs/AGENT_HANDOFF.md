...
- Дата: 2026-09-23. Codex завершил повторный аудит P15 на HEAD **9200a61**, исправления **e4fba39**, ветка `dev/p15-backup-retention`. При начале дерево чистое; проверка на экспорте `/tmp/kz-p15recheck.4uYkW7`.
- **L01 закрыт**: новая/изменённая копия не наследует успех старой, проверяется собственный check и его давность.
- **L02 (P1) открыт**: база с таблицами, но без обязательных рабочих колонок (например, watches.user_id), всё ещё получает restored=True. **L03 (P2) открыт**: зависшее файловое чтение не прерывается по deadline; timeout возникает лишь после возвращения чтения. Критерии и воспроизведения — [P15_AUDIT_CODEX.md](P15_AUDIT_CODEX.md).
- Полный независимый Python suite: **777 тестов, 43.914 с, OK**. Frontend не повторялся: UI в исправлениях не менялся; предыдущие независимые 10/10 PASS относятся к 8b05167. Реальные интеграции, Docker, нагрузка и прод не проверены.
- Независимый просмотр J01–J03/K01/K02 выполнен Claude и описан в предыдущей передаче; это не самоприёмка Codex. Общая пачка ещё не принята из-за P15.
- PR [#2](https://github.com/molthun/kz-price-hunter/pull/2) проверен OPEN, head 9200a61456b15d782da837d1847d519b16540f8b. **Слияние не рекомендовано** до завершения L02/L03; merge запускает выпуск.
- **Следующий точный шаг:** Claude завершает L02/L03, добавляет проверки неполных колонок и зависшего I/O; затем повторный аудит P15. Решение о выпуске — после приёмки.
- Изменены только `docs/P15_AUDIT_CODEX.md` и этот реестр. Код приложения, рабочая БД, PR и прод не менялись; без коммита/push/merge. Фоновых команд/потоков аудита нет.
...
| P15 | IN_PROGRESS | Claude | Codex: L01 закрыт, завершить L02/L03 | Не выпущен |
