...
- Дата: 2026-09-24, Asia/Almaty. Выпуск **v5.17.3** (автор — Antigravity, по прямому поручению владельца).
- **Что вошло в 5.17.3:**
  1) **Динамический cooldown моделей AI:** при получении HTTP 429 (исчерпание квоты / rate limit) модель автоматически исключается из авто-выбора (`auto_model`) и цепочки альтернатив (`alternatives`) на 10 минут (600 с). При HTTP 404 модель исключается на 24 часа. Это устраняет деградацию AI при исчерпании квоты ранних версий моделей (например, `gemini-3.8-flash` с лимитом free tier в 20 запросов), немедленно переводя трафик нормализации на рабочие модели с большим лимитом (`gemini-3.5-flash-lite`, `gemini-3.5-flash`).
  2) **Совместимость с OpenAI:** заменен устаревший параметр `max_tokens` на `max_completion_tokens` (с fallback для старых шлюзов), устраняя HTTP 400 на современных моделях (`gpt-5.4-nano`, `gpt-5.4-mini`, `o1/o3`, `gpt-4o`).
...
- Проверки на текущем снимке: Python **1299 OK**, все **15 frontend-наборов** OK, `release_gate.py secrets` чисто (0 findings).
- **Следующий точный шаг: пуш в main запускает сборку Docker-образа и обновление на проде (Watchtower); проверить /monitoring на проде (статус AI-сервиса и снижение ошибок).**
