...
- **Шлюз выката отработал в живом конвейере с первого раза**: test, frontend, secret-scan,
  dependency-audit (настоящий pip-audit), upgrade-rehearsal (включая запуск кода прежнего релиза на
  обновлённой копии) и image-smoke (образ собран и проверен запущенным контейнером) — все success.
