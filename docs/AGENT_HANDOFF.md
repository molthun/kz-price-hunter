...
- Дата: 2026-09-23 15:07, Asia/Almaty. Исполнитель — Claude.
- Последнее действие: реализован **P11 (контролируемое включение адаптивного планировщика)** на ветке
  `dev/p11-scheduler-rollout` (от main = v5.12.0), коммит `91dd569`. Основание: «Начинай P11».
- **Push не выполнялся, аудита не было.** На проде v5.12.0 (P10/P14/P15), выключатель там в положении «off».
- Решения владельца, согласованные ДО включения (2026-09-23): пороги отката «строго» — рост доли ошибок или
  отказов по лимиту больше чем на треть, падение полноты обходов больше чем на 10 процентных пунктов;
  расширение шага — только вручную владельцем, автоматически — лишь откат.
- Что сделано: шаги off → один дружелюбный магазин → до 5 → все; переключатель в Центре мониторинга
  (раздел «Сканирование») с блокировкой прыжков через шаг; снимок метрик «до» на момент включения и
  сравнение «до/после»; автоматический откат после циклов обхода с записью причины и событием телеметрии;
  настройка `adaptive_scheduler_stage` переживает перезапуск.
- Главная гарантия: адаптивный порядок может только переставить и сократить ту же волну — добавить магазин
  он не может (проверяется утверждением в коде и тестом). Адреса, заголовки и паузы не трогаются.
- Тесты: Python **882 OK** (новый test_scheduler_rollout.py: 41 — шаги в обе стороны, перезапуск, отсутствие
  двойных обходов, пороги отката поимённо, мало данных — не приговор, отказ нового пути не ломает волну),
  frontend **10/10 PASS**. Браузер на стенде: переключатель показывает только допустимые шаги, включение
  и возврат работают; стенд возвращён в «off».
- Не проведено: независимый аудит, настоящий canary на проде, Docker, нагрузка.
- Следующий шаг: аудит P11 (передать Codex), затем — решение владельца о выпуске и о первом реальном шаге
  включения на проде.
