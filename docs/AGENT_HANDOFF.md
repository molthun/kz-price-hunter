...
- Дата: 2026-09-24. Исполнитель — Claude. На проде **v5.14.1**.
- Пробный шаг адаптивного планировщика включён владельцем на `alser` (профиль normal) ещё на 5.13.3;
  наблюдение продолжается. Сводка в Telegram и AI-сопоставление по-прежнему выключены.
- Выпуски после пачки: 5.13.1 (часовые пояса), 5.13.2 (первый шаг на обычном источнике), 5.13.3 (шаг
  видит свой магазин + абсолютные потолки отката), 5.13.4 (видно, почему падает AI), 5.14.0 (список
  моделей от провайдера и работающие цены), 5.14.1 (починка карточки расходов AI).
- **AI на проде сломан у провайдера, не у нас**: gemini даёт 100 % ошибок при нулевых токенах —
  запросы не доходят. Диагноз: имя модели `gemini-2.5-flash` не обслуживается ключом (HTTP 400/404),
  раньше эта ветка молчала в логах. Теперь причина видна в карточке расходов, а модель выбирается из
  списка, который отдаёт сам провайдер. **Действие владельца:** выбрать живую модель в настройках
  (и при желании задать цены — расчёт денег теперь работает).
- **Ошибка 5.11.0, найденная попутно:** заданные цены моделей вырезались при чтении настроек, поэтому
  расход в деньгах не посчитался бы никогда. Исправлено в 5.14.0, закрыто тестами.
- **Сборка 5.14.0 упала на браузерных проверках** (в карточке расходов AI не хватало объявления
  переменной). Шлюз P16 не пустил образ в реестр, прод остался на прежней версии — ровно то, ради чего
  шлюз делался. Причина промаха автора: перед выпуском прогонялись не все браузерные наборы.
  Правило на будущее: перед выпуском гонять все 15 наборов, а не выборочно.
- Проверки на текущем снимке: Python **1256 OK**, frontend **15/15 PASS**, секреты чисто; прод
  подтверждён чтением /api/version (5.14.1) и /monitoring (200).
- **Следующий точный шаг: владелец выбирает рабочую модель в настройках AI** (список приходит от
  провайдера) и при желании задаёт цены. Затем — наблюдение за пробным шагом планировщика и решение о
  расширении до «до 5».
...
| P07 | V1 DEPLOYED (v5.11.0); V2 DEPLOYED (v5.13.0) | Claude | Codex: G01/G02, M10 закрыты | v5.14.1 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть DEPLOYED (v5.13.0) | Claude | Codex: I01/I02, M11 закрыты | v5.14.1 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
...
| P11 | DEPLOYED (v5.13.0) | Claude | Codex: M01–M03 закрыты | v5.14.1 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
| P12 | DEPLOYED (v5.13.0) | Claude | Codex: M04 закрыт | v5.14.1 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
| P13 | DEPLOYED (v5.13.0) | Claude | Codex: M04–M06 закрыты | v5.14.1 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
...
| P16 | DEPLOYED (v5.13.0) | Claude | Codex: M07–M09 закрыты | v5.14.1 на shop.molthun.ru (проверка чтением /api/version, /monitoring) |
