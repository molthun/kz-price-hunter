...
- Дата: 2026-09-24, Asia/Almaty. Выпуск **v5.17.4** (автор — Antigravity, по прямому поручению владельца).
- **Что вошло в 5.17.4:**
  1) **Устранение ложных срабатываний «Деградации» магазинов:** статус магазина в мониторинге теперь определяется по интегральному здоровью каталога, а не по худшей категории. Если магазин собрал полный объем, единичное локальное предупреждение в одной категории больше не красит весь магазин в тревожную «Деградацию» (4mobile, Halyk Market, ITMag остаются «В норме»). При частичном сборе с сохранением основной базы (как Zeta с пустой сезонной категорией дачной мебели) выставляется честный статус «Ограничено». Статус «Деградация» зарезервирован для системных падений: коллапс объёма товаров ниже 50% нормы (Alser: 28 из 121) или массовый сбой категорий $\ge 35\%$ (12 Месяцев: 6 из 8 категорий).
  2) **Общий статус сервиса не объявляет отказ из-за одного магазина:** одиночный сбой магазина (например, Лемана ПРО) больше не перекрашивает весь сервис KZ Price Hunter в красный статус «Недоступен».
  3) **Устойчивость скрапера 12 Месяцев:** добавлен retry при transient сетевых ошибках (HTTP 5xx и таймаутах).
- **Предыдущие релизы дня:**
  - **v5.17.3:** cooldown моделей AI при лимитах и `max_completion_tokens` в OpenAI.
  - **v5.17.2:** починка скрапера Sulpak (AJAX-эндпоинт распродажи, пагинация).
  - **v5.17.1:** починка скрапера Лемана ПРО (обход ServicePipe WAF).
  - **v5.17.0:** наблюдение из карточки товара, кнопка «Не следить» в Telegram, живучесть DNS.
- Проверки на текущем снимке: Python **1300 OK**, все **15 frontend-наборов** OK, `release_gate.py secrets` чисто (0 findings).
- **Починка сборки (Claude, 5.17.5):** все четыре проверки сборки 5.17.4 падали, и на прод не ушли ни 5.17.3, ни 5.17.4 (`build-and-push` пропущен — шлюз сработал правильно).
  - Причина: в `ai_service.py:368` аннотация `_model_cooldown: Dict[Tuple[str, str], float]` без импорта `Tuple`. Локальный venv — Python **3.14**, где аннотации вычисляются лениво (PEP 649), поэтому 1300 тестов были зелёными; CI и образ — Python **3.12**, где аннотация модульного уровня вычисляется при импорте, и `NameError` ронял джобы `test`, `frontend` (тестовый сервер не стартовал) и `image-smoke` (образ не поднимался).
  - **Помните об этом расхождении версий:** локальный прогон не доказывает работоспособность на проде для всего, что касается аннотаций и синтаксиса. Добавлена постоянная статическая проверка `test_import_contract.py`, которая ловит этот класс ошибок на 3.14.
  - Заодно: `clear_model_cooldowns()` был объявлен, но нигде не вызывался — модель, отвергнутая по 404, исключалась на сутки без способа это снять. Теперь сохранение настроек AI снимает исключения (тест `test_saving_ai_settings_clears_model_cooldowns`).
- Проверки на текущем снимке: Python **1302 OK**, все **15 frontend-наборов** OK, `release_gate.py secrets` чисто (0 findings).
- **Подъём версий ПО (Claude, 5.18.0):** по задаче владельца «все версии ПО поднимаем до последних стабильных».
  - Python **3.12 → 3.14** в `ci.yml`, `docker-publish.yml` (7 мест) и в образе. Расхождение локальной 3.14 и CI/образа 3.12 закрыто — именно оно стоило упавшей сборки 5.17.4.
  - Базовый образ `playwright/python:v1.49.0-jammy` → **`v1.63.0-resolute`** (Ubuntu 26.04 LTS, штатный Python 3.14.3 — проверено по Launchpad). Закрыт отложенный после аудита пункт M11 о расхождении образа (1.49) и библиотеки (1.63).
  - Перед правкой проверено по PyPI: для всех бинарных зависимостей есть колёса `cp314` под linux x86_64, компиляции из исходников в сборке не будет.
  - `pip install` в образе получил `--break-system-packages`: Ubuntu 26.04 помечает системный Python как externally-managed (PEP 668), а `CMD` и `HEALTHCHECK` зовут системный `python3`.
  - Зависимости: прямые уже были на последних версиях; `multidict` 6.9.0 → 6.9.1, `soupsieve` 2.9.2 → 2.10, `tzdata` закреплена на 2026.4 (стояла без версии). `pyee` остаётся 13.0.1 — потолок playwright 1.63.
  - Все действия Actions закреплены по SHA (в `ci.yml` оставались `@v4`/`@v5`), Node 22 → 24 LTS.
- **Чего проверка не покрывает и что смотреть после выката:** локального Docker нет, образ собирает и проверяет только CI (`image-smoke`). Новый образ несёт другой Chromium — под наблюдением **DNS** и **«Лемана ПРО»**, оба ходят через Playwright. Поведение браузера на живых сайтах шлюз не проверяет.
- Проверки на текущем снимке: Python **1302 OK** на 3.14 с новыми пинами, **15 frontend-наборов** OK, `compileall` чисто, `release_gate.py secrets` чисто.
- **Следующий точный шаг: дождаться зелёной сборки 5.18.0 и раскатки, затем проверить на /monitoring статус Sulpak, DNS и «Лемана ПРО».**
