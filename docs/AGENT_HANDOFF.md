...
- Исполнитель — Codex, приёмка доработки. Проверен HEAD `8bb9480e07e8c8313f4384142023ac45147bbdd7`, код `3f88c5f`, ветка `dev/p07-watch-v2`. Дерево до аудита чистое.
- **Пачка остаётся IN_PROGRESS. M02/M06/M07 закрыты в проверенном объёме; M01/M03/M04 остаются открыты.** M05/M08/M09/M10/M11 сохраняют предыдущую приёмку с её ограничениями.
- M01: пустой allowed=[] разрешает всех; выключение всех магазинов не отмечает сохранённого участника как недоступного.
- M03: отказ одного из двух конкурентных переключений стирает metadata успешного второго; воспроизведено stage=canary_one при baseline=None и members=[]. Последовательный отказ исправлен.
- M04: ответ «За сутки было {http.requests} ошибок» при errors=0/requests=50 превращается в «50 ошибок» и не отклоняется. Название метрики по-прежнему задаёт модель.
- Независимо подтверждены: дельта первого часа и откат при 100% ошибок; partial для смешанной стоимости; upgrade --keep → rollback старым кодом по обновлённой копии с неизменным источником.
- Полные проверки: **1151 Python-тест, 51.856 с, OK; 14 браузерных наборов успешно**. Временные БД/настройки, mock AI/Telegram. Экспорт `/tmp/kz-remainders.9qCHTR`; все команды и потоки завершены.
- Отчёт и точные критерии: [BATCH_AUDIT_CODEX.md](BATCH_AUDIT_CODEX.md), начало файла. Docker, прод, живой CI/pip-audit и реальные интеграции не проверялись.
- Изменены только `docs/BATCH_AUDIT_CODEX.md` и этот реестр. Без правок приложения, коммита, push, merge или выпуска.
- **Следующий точный шаг: Claude исправляет M01/M03/M04 с регрессиями и возвращает на повторную приёмку.** Выпуск пока не рекомендован; другой агент автоматически не запускался.
...
| P07 | V1 DEPLOYED (v5.11.0); V2 READY (аудит кода) | Claude | Codex: G01/G02 закрыты (V1); M10 закрыт для пакета 25 событий | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть READY (аудит кода; живой замер не выполнен) | Claude | Codex: I01/I02 закрыты (правила); M11 закрыт; on до живого замера не включать | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
| P11 | IN_PROGRESS | Claude | Codex: M02 закрыт; M01/M03 открыты | Не выпущен |
| P12 | IN_PROGRESS | Claude | Codex: M04 открыт | Не выпущен |
| P13 | IN_PROGRESS | Claude | Codex: M05/M06 закрыты; M04 открыт | Не выпущен |
...
| P16 | READY (аудит кода) | Claude | Codex: M07–M09 закрыты в проверенном объёме; живой CI не проверен | Не выпущен |
