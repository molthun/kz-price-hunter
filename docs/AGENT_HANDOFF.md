...
- Дата: 2026-09-24, Asia/Almaty. Исполнитель разработки — **Antigravity** (подхватил сессию после Claude). На проде **v5.15.0**.
...
- **Состояние AI на проде:** gemini отвечал 404 на `gemini-2.5-flash`; причина видна в карточке
  («Почему падает»). Владелец меняет модель из списка — теперь выбор сам включает ручной режим, и
  карточка показывает действующую модель. Цены можно подставить кнопкой из прайса и подтвердить
  сохранением; расчёт денег работает (до 5.14.0 цены терялись при чтении настроек).
- Ограничения подсказки цен: прайс OpenAI рисуется в браузере и сервером не читается — там только
  ручной ввод; модели с ценой по длине запроса или виду данных помечаются как неоднозначные.
- Ошибки, найденные владельцем за сессию (все исправлены): отсутствие tzdata ломало сохранение
  настроек; требование FRIENDLY для первого шага недостижимо; шаг не видел HTTP-метрики своего
  магазина; откат без снимка «до» не срабатывал; выбранная модель не применялась в режиме «Авто».
  Общая причина: проверка в окружении разработчика и на синтетических данных.
- Проверки на текущем снимке: Python **1267 OK**, frontend **15/15 PASS**, секреты чисто; прод —
  /api/version 5.15.0, /monitoring 200, админский API без входа 401.
- **Следующий точный шаг: владелец выбирает живую модель и подтверждает цены**, затем — наблюдение за
  пробным шагом планировщика и решение о расширении до «до 5».
