...
- Последнее действие: реализован **P12 (помощник администратора)** на ветке `dev/p12-admin-assistant`
  (от main = v5.12.0), коммит `5f50c44`. Основание: «Давай дальше кодекс пока не сможет сделать аудит.
...
- P11 остаётся на своей ветке `dev/p11-scheduler-rollout` (коммит `91dd569`); P12 сделан от main отдельно.
...
Ветка / базовый HEAD: dev/p12-admin-assistant / 7375c91 (main, v5.12.0). Коммит: 5f50c44.
Что сделано:
