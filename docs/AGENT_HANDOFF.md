...
- Дата: 2026-09-23 14:45, Asia/Almaty. Исполнитель — Claude.
- Последнее действие: подготовлен выпуск **5.12.0** (P10+P14+P15) по поручению владельца «Подготовить выпуск».
  Версия поднята, CHANGELOG/ROADMAP/README обновлены; изменения лежат в ветке `dev/p15-backup-retention`
  и в PR [#2](https://github.com/molthun/kz-price-hunter/pull/2).
- Аудит: пачка принята Codex (J01–J03, K01/K02, L01–L04 закрыты). Исправления по P10/P14 писал Codex, их
  независимо просмотрел Claude — запись выше в истории передачи.
- Релизные проверки на копии рабочей базы (копия в scratchpad; рабочая `prices.db` не тронута):
  - Python **841 OK**, frontend **10/10 PASS**.
  - Старт 5.12.0 на копии: `/api/version` → 5.12.0, `schema_version` остался **5**, главная, `/monitoring`
    и `/static/js/format.js` отвечают 200.
  - Витрина на копии пуста — это правило свежести P02, а не регрессия: локальная база не обновлялась с
    19.09, свежих цен за 72 ч в ней нет. На проде данные свежие.
  - **Откат проверен**: код v5.11.0 запущен на той же базе — отвечает 5.11.0, главная и мониторинг 200,
    ошибок в логе нет. Восстановление БД для отката не требуется.
- Не проверено: Docker-образ, реальные обходы магазинов, вызовы провайдеров AI, отправка в Telegram, прод
  (на нём сейчас v5.11.0), нагрузка.
- **Слияние PR = выпуск**: push в main запускает сборку образа и автодеплой (~5 мин). Выполняется только по
  отдельному слову владельца.
- Следующий шаг — по решению владельца: «выпускай» (слить PR) либо следующий этап (P11, P12, P13, P16).
...
| P10 | READY (в выпуске 5.12.0) | Claude → Codex | Claude: просмотр исправлений J01–J03 без замечаний | Не выпущен |
| P11 | TODO | — | Не проведён | Не проверен |
| P12 | TODO | — | Не проведён | Не проверен |
| P13 | TODO | — | Не проведён | Не проверен |
| P14 | READY (в выпуске 5.12.0) | Claude → Codex → Claude | Claude: просмотр исправлений K01/K02; совместный suite OK | Не выпущен |
| P15 | READY (в выпуске 5.12.0) | Claude | Codex: L01–L04 закрыты в проверенном объёме | Не выпущен |
