...
- Дата: 2026-09-23 18:30, Asia/Almaty. Исполнитель — Claude.
- Последнее действие: реализован **P16 (шлюз выката)** на ветке `dev/p16-ci-gate`, коммит `2f3effa`.
  Этим пачка, заказанная владельцем, закрыта: P11 → P12 → P13 → P16.
- **Push не выполнялся, аудита не было.** На проде v5.12.0.
- Пачка — одна линия истории: P11 (`91dd569`) → P12 (`5f50c44`) → P13 (`fe7169e`) → P16 (`2f3effa`).
- Что сделано в P16: release_gate.py (поиск секретов, разбор отчёта pip-audit со списком пропусков,
  репетиция обновления базы предыдущего релиза, smoke по работающему образу) и переписанный
  docker-publish.yml: публикация зависит от заданий test, frontend, secret-scan, dependency-audit,
  upgrade-rehearsal, image-smoke; образ собирается и проверяется ДО публикации. Процедура выката и
  возврата — docs/RELEASE_AND_ROLLBACK.md.
- Проверено по-настоящему, а не только моками: репетиция обновления прогнана локально на базе,
  созданной кодом тега v5.12.0 (схема 5 → 5, все запросы приложения проходят, целостность ok, возврат
  без восстановления копии); smoke прогнан против настоящего приложения — из-за него ожидания правок:
  закрытым разделам засчитывается и 401, и 403.
- Тесты: Python **950 OK** (новый test_release_gate.py: 30), frontend **12/12 PASS**.
- Не проведено: независимый аудит; настоящий прогон конвейера в GitHub Actions (будет при первом push);
  сборка образа и контейнерный smoke на этой машине (Docker-операций не делаю по правилу владельца);
  намеренная поломка проверки в реальном конвейере — процедура описана в docs/RELEASE_AND_ROLLBACK.md.
- Следующий шаг: передать всю пачку (P11, P12, P13, P16) Codex на аудит; после приёмки — решение владельца
  о выпуске и о первом реальном шаге включения P11 на проде.
...
| P07 | DEPLOYED (v5.11.0) | Claude | Codex: G01/G02 закрыты | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0) | Claude | Codex: I01/I02 закрыты; AI-часть отдельно | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
Не проведено: независимый аудит, прод, Docker, настоящая модель (провайдер подменён в тестах),
  отправка сводки в Telegram — по плану отдельное последующее включение.
```
