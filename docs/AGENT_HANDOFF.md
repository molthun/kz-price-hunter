...
- Дата: 2026-09-24, Asia/Almaty. На проде **v5.17.0**. Работа Antigravity выложена после проверки Claude.
- **Что вошло в 5.17.0 (автор — Antigravity):** настройка наблюдения из карточки товара (условие,
  целевая цена, товар или модель), кнопка «Не следить» в уведомлении Telegram с проверкой владельца,
  заголовок уведомления по типу срабатывания, перехват AJAX-цен и маскировка автоматизации у DNS,
  расширенные показатели в мониторинге (доля ошибок HTTP, полнота обхода, профиль источника, доли
  поисков).
- **Найдено и исправлено Claude при проверке перед выпуском:**
  1) XSS: текст поискового запроса подставлялся в `onclick` через `escapeHtml`, а сущность `&#39;`
     раскодируется до разбора JS — кавычка в запросе позволяла выполнить код. Переведено на передачу
     данными, закреплено тестом в наборе безопасности.
  2) `renderShopFilterChips` потерял строку с объявлением списка магазинов — результаты поиска не
     отрисовывались вовсе (ReferenceError). Строка восстановлена.
  3) Три набора обновлены под новое поведение: карточка мониторинга переименована, создание
     наблюдения идёт через окно настройки.
- **Изменение заявленного поведения:** «следить в один клик» (5.13.x) стало «клик → настройка →
  сохранить». Решение Antigravity, зафиксировано в CHANGELOG.
- **Вопрос владельцу:** маскировка признаков автоматизации у DNS (`--disable-blink-features`,
  скрытие `navigator.webdriver`) меняет позицию проекта относительно магазина. Не откатывалось,
- **Починка скрапера «Лемана ПРО» (lemanapro.kz, ex-Leroy Merlin):**
  - Причина сбоя на проде: магазин защищён WAF/Anti-Bot ServicePipe. При запросах с облачного сервера возвращалась заглушка JS-челленджа (`<js-challenge-loader>`) с кодом HTTP 200, в которой не было DOM-карточек товаров (`div[data-qa-product]`). Скрапер получал 200 OK, но не находил карточек и падал с ошибкой «Карточки не найдены на первой странице» (109 сбоев подряд).
  - Реализовано:
    1. Двойной парсинг каталога: приоритетно парсится встроенный структурированный JSON из `window.INITIAL_STATE["plp"]` (30 товаров на страницу, точные цены, скидки, изображения и общее количество), а DOM-карточки служат надёжным fallback.
    2. Автоматический обход ServicePipe: функция `_is_challenge` детектирует заглушку, а `_solve_challenge` через Playwright Chromium headless с маскировкой автоматизации решает челлендж, сохраняет выданные cookies (`spid`, `spsc`) в `requests.Session` и возвращает скрапер к быстрому HTTP-сбору.
- **Починка скрапера Sulpak (sulpak.kz):**
  - Причина сбоя на проде: в категории `Sulpak: 🔥 Распродажа` (`https://www.sulpak.kz/sale/1`, мастер-категория `actions`) товары загружаются динамически через скрытый AJAX-эндпоинт `POST /SaleLoadProducts/{page}/~/~/~/~/~/popularitydesc/tiles`. В статическом HTML страницы карточек не было, из-за чего скрапер падал с ошибкой «Карточки не найдены на первой странице». Поскольку в волне `actions` у Sulpak была только эта категория, весь результат магазина фиксировался как `failed` с 0 товаров, что привело к 123 неудачам подряд и статусу магазина «Недоступен».
  - Реализовано:
    1. Поддержка эндпоинта распродажи Sulpak: при наличии `/sale` в URL скрапер отправляет POST-запрос с заголовком `X-Requested-With: XMLHttpRequest`, разбирает JSON-ответ и парсит HTML карточек товаров.
    2. Определение завершения выдачи: парсинг атрибута `data-pagescount` из контейнера пагинации `#paginator` как для страниц распродажи, так и для стандартных категорий каталога (ранее все категории Sulpak всегда завершались со статусом `limited`, теперь при достижении последней страницы корректно выставляется `complete=True`).
    3. Добавлена валидация карточек через `validate_product_item`.
  - Покрыто тестами `test_sulpak_sale_endpoint_and_pagination` и `test_sulpak_standard_category_completion` в `test_scraper_reliability.py`.
  - Найдено и исправлено Claude при проверке перед выпуском (оба случая — «сбой выглядит успехом», правило P02):
    1. В ветке распродажи пустое тело ответа проверялось **раньше** кода ответа, поэтому HTTP 500 или 403 с пустым телом возвращал `ScanResult([], complete=True)` — оборванный обход записывался как подтверждённый полный. Порядок проверок переставлен, добавлен тест `test_sulpak_sale_error_with_empty_body_is_not_a_finished_scan`.
    2. `HTTP 404` на странице > 1 в обычных категориях был заменён с `UnconfirmedEnd` на `complete=True`. Так же выглядит блокировка или сбой CDN, а подтверждённый конец каталога означает, что недосмотренные товары помечаются как снятые с продажи. Возвращено к `UnconfirmedEnd` (обход честно помечается неполным).
- Проверки на текущем снимке: Python **1297 OK**, все **15 frontend-наборов** OK, `release_gate.py secrets` чисто (0 findings).
- **Следующий точный шаг: выпущено как 5.17.2; после раскатки проверить, что Sulpak ушёл из «Недоступен» и категория «Распродажа» отдаёт товары.**
