...
- Дата: 2026-09-23 16:40, Asia/Almaty. Исполнитель — Claude.
- Последнее действие: реализован **P12 (помощник администратора)** на ветке `dev/p12-admin-assistant`,
  коммит `5f50c44`. Ветка продолжает `dev/p11-scheduler-rollout` (от `e8fba6f`), то есть содержит и P11:
  пачка накапливается одной линией истории, а не независимыми ветками. Основание: «Давай дальше кодекс пока не сможет сделать аудит.
  сделаем всю пачку он позже проверит» — копим пачку локально, аудит Codex позже.
- **Push не выполнялся, аудита не было** ни по P11, ни по P12. На проде v5.12.0, выключатель P11 там в «off».
- В пачке уже два этапа: P11 (`91dd569`) и P12 (`5f50c44`). Аудит Codex будет по всей пачке сразу.
- Что сделано в P12: admin_assistant.py — закрытый список из девяти инструментов только на чтение (магазины,
  обходы и HTTP, инциденты, аналитика поиска, расходы AI, планировщик, качество карточек, окружение, копии);
  подбор инструментов по вопросу детерминирован; у каждого блока фактов есть источник и период; ответ модели
  с числом, которого нет в данных, отклоняется и показывается сухая сводка; панель в Центре мониторинга
  (раздел «Обзор»), POST /api/admin/assistant под require_admin.
- Главная гарантия: помощник ничего не меняет — ни одного вызова записи в исходниках инструментов
  (проверяется тестом), а цифры в ответе всегда сверяются с собранными фактами.
- Тесты: Python **899 OK** (новый test_admin_assistant.py: 17 — падение каталога dns 2000 → 100 и рост 4xx,
  выдуманные числа, отсутствие данных, injection в названии категории, права 401/403/200/400, работа без AI),
  frontend **11/11 PASS** (новый test_frontend_p12.cjs).
- Не проведено: независимый аудит, проверка на проде, Docker, работа с настоящей моделью (в тестах провайдер
  подменён), оценка качества формулировок ответа на живых данных.
- Следующий шаг: продолжать пачку (P13, затем P16) по поручению владельца; аудит Codex по всей пачке позже.
