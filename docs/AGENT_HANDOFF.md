...
- Предыдущие выпуски дня: 5.13.1 … 5.16.1 (часовые пояса, шаг планировщика, список моделей и цены,
  режим «Авто» по списку провайдера, починка вызовов Gemini).
- Проверки перед выпуском: Python **1292 OK**, frontend **15/15 PASS**, секреты чисто; прод —
  /api/version 5.17.0, /api/stats 200, /monitoring 200.
- **Следующий точный шаг:** владелец смотрит карточку AI (действующая модель и ошибки), подтверждает
  цены моделей и наблюдает пробный шаг планировщика на `alser` для решения о расширении до «до 5».
