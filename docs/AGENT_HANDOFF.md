...
- Дата: 2026-09-24. Исполнитель — Claude. На проде **v5.13.3**.
- **Пробный шаг адаптивного планировщика включён владельцем заново на 5.13.3**: `canary_one`,
  участник `alser` (профиль normal), снимок «до» снят уже исправленным кодом. Наблюдение началось.
- Выпуски сессии: v5.13.0 (пачка после аудита), v5.13.1 (регрессия часовых поясов), v5.13.2 (первый
  шаг возможен на обычном источнике), v5.13.3 (шаг видит свой магазин, абсолютные потолки отката).
  Все собраны конвейером со шлюзом P16 полностью, версии подтверждены чтением /api/version.
- **Что нашёл владелец, а не тесты и не аудит** (записано, чтобы не повторять):
  1) в образе нет базы часовых поясов — падало сохранение любых настроек (5.13.1);
  2) «первый шаг только на дружелюбном источнике» недостижимо на реальных данных (5.13.2);
  3) обходы хранятся под ключом настроек, а HTTP-метрики под названием магазина — шаг не видел ни
     одного запроса, автооткат был слеп (5.13.3);
  4) при этом же разборе вскрылось: откат зависел от снимка «до», поэтому без прежних HTTP-наблюдений
     даже 90 % ошибок ничего не запускали — добавлены абсолютные потолки 20 % ошибок и 5 % отказов.
  Общая причина первых трёх: проверка шла в окружении разработчика и на синтетических данных.
- Известное ограничение текущей пробы: у `alser` полнота обходов 0 % и до, и после включения (нет
  обходов с качеством «ok»), поэтому сигнал «полнота упала» на нём не сработает. Защита по ошибкам и
  отказам работает, в том числе по абсолютным потолкам.
- **Следующий точный шаг: через сутки наблюдения посмотреть карточку включения.** Смотреть строки
  «Наблюдений после включения» (нужно 50 запросов и 5 обходов), «Доля ошибок», «Отказы по лимиту» и
  «Готов следующий шаг». Если запросов не набирается — выбрать для пробы более активный магазин.
  Дальше — решение владельца о расширении до «до 5». Сводка в Telegram и AI-сопоставление
  по-прежнему выключены; до режима `on` у AI-части обязателен замер на живой модели.

## Проверка подхвата Codex — 2026-09-24 10:41 (Asia/Almaty)

- Область работы: чтение состояния прода; запись checkpoint только в `docs/AGENT_HANDOFF.md`.
  Ветка `main`, HEAD `b091f51f3789f032bfaa95139211e58436418f01`; перед записью checkout чистый.
- `/api/version` подтвердил **5.13.3**. В авторизованном Safari прочитана карточка
  `/monitoring#scanning`: «В норме — один магазин на пробу», `alser (normal)`, «Идёт с: 6 мин назад».
- Наблюдения после включения: **3 запроса из 50, 1 обход из 5**.
  Доля ошибок **0.6 % → 0.0 %**, отказы по лимиту **0.0 % → 0.0 %**,
  полнота обходов **0.0 % → 0.0 %**. «Готов следующий шаг: нет — наблюдений пока мало».
  Это начальное наблюдение, а не подтверждение устойчивости за сутки или проверка срабатывания отката.
- Расхождение с предыдущей записью: на странице обзора флажок ежедневной сводки
  «присылать в Telegram» был включён. Утверждение выше о выключенной сводке устарело;
  фактическая доставка не проверялась. Режим AI-сопоставления в этой проверке не проверялся.
- Настройки прода, состав пробного этапа и флаги не изменялись; вызовы AI и рассылки не запускались.
- Следующий точный шаг: **25.09.2026 около 10:41 Asia/Almaty**, при условии отсутствия нового
  перезапуска, повторно прочитать эту карточку: длительность, запросы/обходы, ошибки, отказы,
  полноту и готовность. При недостатке запросов предложить владельцу более активный магазин;
  расширение до «до 5» остаётся решением владельца. Автоматическая повторная проверка не назначена.
...
| P07 | V1 DEPLOYED (v5.11.0); V2 DEPLOYED (v5.13.0) | Claude | Codex: G01/G02, M10 закрыты | v5.13.3 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть DEPLOYED (v5.13.0) | Claude | Codex: I01/I02, M11 закрыты | v5.13.3 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P11 | DEPLOYED (v5.13.0) | Claude | Codex: M01–M03 закрыты | v5.13.3 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
| P12 | DEPLOYED (v5.13.0) | Claude | Codex: M04 закрыт | v5.13.3 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
| P13 | DEPLOYED (v5.13.0) | Claude | Codex: M04–M06 закрыты | v5.13.3 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P16 | DEPLOYED (v5.13.0) | Claude | Codex: M07–M09 закрыты | v5.13.3 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
