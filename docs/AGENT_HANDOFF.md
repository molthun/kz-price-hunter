...
- Дата: 2026-09-24 02:30, Asia/Almaty. Исполнитель — Клод. На проде **v5.13.1**.
- Выпущены v5.13.0 (пачка P11, P12, P13, P16, P07 V2, AI-часть P09 после приёмки аудита) и следом
  v5.13.1 — исправление регрессии, найденной владельцем при первой попытке включить пробный шаг.
- **Регрессия 5.13.0 и её разбор:** добавленная проверка часового пояса суточной сводки падала в образе,
  где нет базы часовых поясов IANA (`ZoneInfoNotFoundError` даже на «UTC»). Из-за этого ломалось любое
  сохранение системных настроек, поэтому переключатель шага отвечал «Шаг не включён», а суточный отчёт
  не собирался. Тихие часы наблюдений падали по той же причине ещё с P07, но молча.
- Исправление: `tzdata` объявлена зависимостью; `timezones.py` даёт UTC вместо исключения; «не могу
  проверить имя» больше не читается как «имя неверное»; переключатель шага называет причину отказа и
  повторяет попытку при занятой базе. Новый набор `test_timezones.py` проверяет работу без tzdata.
- Почему не поймали раньше: и аудит, и все прогоны шли в окружении, где tzdata есть. Проверки поведения
  в более бедном окружении не было — теперь она в наборе.
- Конвейер: шлюз выката отработал на обоих выпусках полностью (test, frontend, secret-scan,
  dependency-audit, upgrade-rehearsal, image-smoke, build-and-push — success).
- Прод проверен чтением (02:30): `/api/version` — 5.13.1 от 2026-09-24; `/api/stats` — 94 893 товара,
  16 773 предложения с выгодой, 437 арбитражных; `/monitoring` — 200; `/api/admin/monitoring` без
  входа — 401.
- Состояние возможностей: шаг адаптивного планировщика `off`, отправка сводки в Telegram выключена,
  режим AI-сопоставления `off`.
- **Следующий точный шаг: владелец повторяет включение `canary_one` в Центре мониторинга.** Если шаг
  снова откажет, интерфейс теперь показывает причину — её и разбирать. После включения — сутки
  наблюдения, затем решение о расширении до «до 5».
...
| P07 | V1 DEPLOYED (v5.11.0); V2 DEPLOYED (v5.13.0) | Claude | Codex: G01/G02, M10 закрыты | v5.13.1 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть DEPLOYED (v5.13.0) | Claude | Codex: I01/I02, M11 закрыты | v5.13.1 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P11 | DEPLOYED (v5.13.0) | Claude | Codex: M01–M03 закрыты | v5.13.1 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
| P12 | DEPLOYED (v5.13.0) | Claude | Codex: M04 закрыт | v5.13.1 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
| P13 | DEPLOYED (v5.13.0) | Claude | Codex: M04–M06 закрыты | v5.13.1 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P16 | DEPLOYED (v5.13.0) | Claude | Codex: M07–M09 закрыты | v5.13.1 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
