...
- Дата: 2026-09-24 03:00, Asia/Almaty. Исполнитель — Claude. На проде **v5.13.2**.
- За сессию выпущены: v5.13.0 (пачка после приёмки аудита), v5.13.1 (регрессия часовых поясов),
  v5.13.2 (первый шаг включения планировщика снова возможен). Все три собраны конвейером со шлюзом
  P16 целиком; прод подтверждён чтением /api/version.
- **Две ошибки, найденные владельцем при попытке включить пробный шаг, и их корни:**
  1) проверка часового пояса падала в образе без базы IANA и ломала сохранение ЛЮБЫХ системных
     настроек (5.13.1: пакет `tzdata` + `timezones.py`, деградация к UTC вместо исключения);
  2) условие «первый шаг только на дружелюбном источнике» недостижимо на реальных данных — таких
     источников у магазинов Казахстана нет (5.13.2: допустим и обычный источник; медленные, тяжёлые и
     отвечающие ошибками по-прежнему запрещены; профиль выбранного магазина виден в карточке).
  Обе ошибки прошли мимо аудита и 1191 теста, потому что проверялись в окружении с tzdata и на
  синтетических кандидатах с профилем FRIENDLY.
- Состояние возможностей: шаг адаптивного планировщика `off` (владелец повторяет включение),
  отправка суточной сводки в Telegram выключена, режим AI-сопоставления `off`.
- **Следующий точный шаг: владелец нажимает «1 магазин» в Центре мониторинга.** Ожидается включение с
  показом профиля пробного магазина либо названная причина отказа. Через сутки наблюдения — решение о
  расширении до «до 5». Замер AI-части на живой модели по-прежнему обязателен до режима `on`.
...
| P07 | V1 DEPLOYED (v5.11.0); V2 DEPLOYED (v5.13.0) | Claude | Codex: G01/G02, M10 закрыты | v5.13.2 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть DEPLOYED (v5.13.0) | Claude | Codex: I01/I02, M11 закрыты | v5.13.2 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P11 | DEPLOYED (v5.13.0) | Claude | Codex: M01–M03 закрыты | v5.13.2 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
| P12 | DEPLOYED (v5.13.0) | Claude | Codex: M04 закрыт | v5.13.2 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
| P13 | DEPLOYED (v5.13.0) | Claude | Codex: M04–M06 закрыты | v5.13.2 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
...
| P16 | DEPLOYED (v5.13.0) | Claude | Codex: M07–M09 закрыты | v5.13.2 на shop.molthun.ru (проверка чтением /api/version, /api/stats, /monitoring) |
