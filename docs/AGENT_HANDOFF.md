...
- Дата: 2026-09-23 21:30, Asia/Almaty. Исполнитель — Claude.
- Последнее действие: реализована **AI-часть P09** (модель разбирает только спорные пары сопоставления),
  коммит `3832dcc` на ветке `dev/p07-watch-v2`. Этим все пункты плана, включая отложенные, закрыты.
- **Push не выполнялся, аудита не было.** На проде v5.12.0.
- Пачка: P11 (`91dd569`) → P12 (`5f50c44`) → P13 (`fe7169e`) → P16 (`2f3effa`) → P07 V2 (`24dfc16`)
  → сводка в Telegram (`17984ce`) → AI-часть P09 (`3832dcc`).
- Решение владельца, согласованное ДО замеров (2026-09-23, вариант «строго»): ответ модели принимается при
  уверенности ≥ 0.90; включать AI-путь можно только если на проверочном наборе точность осталась 1.0,
  а полнота выросла минимум до 0.94. Записано в catalog_ai.py и docs/golden_matching.json.
- Что сделано: catalog_ai.py (may_ask — только спорные пары, parse/accepted, resolve через ai_router с
  кэшем на сутки, resolve_pending порциями, evaluate_with_ai с проверкой согласованных порогов);
  таблица matching_pairs (очередь спорных пар и решения, срок хранения 90 дней в политике P15);
  режим ai_matching_mode off/shadow/on по умолчанию off; разбор — в обслуживании после волны, на пути
  сравнения цен сетевых вызовов нет; блок AI в карточке сопоставления мониторинга и
  POST /api/admin/monitoring/matching/ai.
- Главная гарантия: модель может только разрешить сравнение там, где правило воздержалось. Противоречие
  фасовки ей не показывается, признанный товар не пересматривается, сбой AI-части ничего не меняет.
- Тесты: Python **1047 OK** (новый test_catalog_ai.py: 34 — границы полномочий, пороги, разбор ответа,
  очередь и её предел, влияние на сравнение в каждом режиме, права), frontend **14/14 PASS**
  (новый test_frontend_p09_ai.cjs).
- Не проведено: независимый аудит, прод, Docker, **замер на живой модели** (нужен ключ владельца; до
  замера режим остаётся off/shadow), оценка расхода на реальном потоке спорных пар.
- Следующий шаг: передать пачку Codex на аудит, когда он сможет. Незакрытых пунктов плана не осталось.
...
| P07 | V1 DEPLOYED (v5.11.0); V2 REVIEW (аудита не было) | Claude | Codex: G01/G02 закрыты (V1) | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
| P09 | DEPLOYED (v5.11.0); AI-часть REVIEW (аудита не было) | Claude | Codex: I01/I02 закрыты (правила) | v5.11.0 на shop.molthun.ru с 18:26 (проверка чтением /api/version, /api/stats, /api/deals, /monitoring) |
...
| P11 | REVIEW (реализовано, аудита не было) | — | Не проведён | Не проверен |
| P12 | REVIEW (реализовано, аудита не было) | Claude | Не проведён | Не проверен |
| P13 | REVIEW (реализовано, аудита не было) | Claude | Не проведён | Не проверен |
...
| P16 | REVIEW (реализовано, аудита не было) | Claude | Не проведён | Не проверен |
