...
- Исполнитель — Codex, приёмка HEAD `941172589d4c24241a9d89b31689563c09ffeffa` (код `ad9da38`), ветка `dev/p07-watch-v2`. До аудита дерево чистое.
- **M01/M04 закрыты в проверенном объёме. M03 остаётся открыт: сбой записи off при автоматическом rollback оставляет canary_one включённым, но удаляет состав и снимок HTTP-счётчиков. Пачка IN_PROGRESS.**
- Прежняя гонка двух switch_stage исправлена: второй поток ждёт завершения первого, отказ первого не стирает успешное состояние второго. Новый воспроизводимый остаток находится в rollback, который очищает metadata до записи settings.json без восстановления при ошибке.
- Независимые проверки: **1177 Python-тестов, 56.977 с, OK**. Все **14 браузерных наборов имеют успешный результат**, но P07 V2 на первом проходе упал на обращении к ещё не заполненному calls.at(-1); повтор и оставшиеся наборы прошли. Нестабильность теста отмечена в отчёте.
- Сценарии M01 с пустым allowed и M04 со ссылкой под ложной подписью повторены; оба исправлены. AI/Telegram только mocks, данные временные. Все команды/потоки завершены.
- Отчёт: [BATCH_AUDIT_CODEX.md](BATCH_AUDIT_CODEX.md), начало файла. Остальные замечания сохраняют прежнюю приёмку и ограничения. Прод, Docker и живые интеграции не проверялись.
- Правки только документов аудита и передачи; код, рабочая БД и прод не менялись. Без коммита, push, merge или выпуска.
- **Следующий точный шаг: Claude исправляет остаток M03 в rollback с регрессиями отказа записи и конкуренции; затем повторная приёмка.** Выпуск пока не рекомендован. Другой агент не запускался.
...
| P11 | IN_PROGRESS | Claude | Codex: M01/M02 закрыты; остаток M03 в rollback открыт | Не выпущен |
| P12 | READY (аудит кода) | Claude | Codex: M04 закрыт в проверенном объёме | Не выпущен |
| P13 | READY (аудит кода) | Claude | Codex: M04–M06 закрыты в проверенном объёме | Не выпущен |
