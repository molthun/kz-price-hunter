...
- **Наблюдение за обращениями магазинов к парсерам, релиз v5.24.0 (Claude, по поручению владельца):**
  - Владелец нашёл в консоли shop.kz обращение «Если вы парсите сайт… воспользуйтесь готовой выгрузкой» со ссылкой на YML и попросил проверить остальных и поставить это под наблюдение.
  - Новый `shop_notices.py` + таблица `shop_notices` (добавочная, `schema_version` прежний) + оповещение через общую очередь; подключено в `notifier.py` рядом с суточной сводкой, раз в неделю. Список магазинов берётся из `SHOP_REGISTRY`, поэтому новый магазин попадает под наблюдение сам.
  - Сообщение shop.kz живёт **не в HTML, а в собранном JS-бандле**, поэтому проверяются и свои скрипты страницы. Разбор идёт от ключевого слова к границам литерала: на бандле в 400 КБ обычный разбор на строки теряет фразу.
  - **Главная сложность — ложные срабатывания.** «crawler»/«scraper» на сайтах магазинов встречаются в списках user-agent библиотек определения ботов, в CSS-классах (`subscraption`) и атрибутах (`data-is-crawler`). Тесты построены на этих настоящих примерах с живых сайтов. Проверено вживую на всех 30 магазинах: **1 настоящее попадание, 0 ложных, 1 честно «не проверен»** (moon.kz — ошибка сертификата).
  - Не сделано намеренно: раздела в Центре мониторинга нет, потому что `web/server.py` правит Antigravity. Оповещение работает и без него.
- **Разбор «почему четыре магазина собирают мало» и релиз v5.23.1 (Claude, по поручению владельца):**
  - **Тихой поломки нет ни у одного.** DNS: `max_pages: 1` во всех 13 категориях, и это правильно — страница 2 отдаёт **HTTP 403**, при росте max_pages каждая категория DNS начнёт сообщать об ошибке. Больше с DNS можно взять только через обход блокировки, как сделано для «Лемана ПРО». Alser: каталог действительно крошечный, сайт сам печатает «10 товаров» и столько же отдаёт его API. Магнум: magnum.kz online — витрина акций, 138 уникальных, обход подтверждает полноту.
  - **Настоящий дефект оказался другим:** Alser и MasterOK не могли отчитаться `complete` ни при каком размере каталога → baseline не строился, мониторинг вечно показывал «собран не полностью». Исправлено чтением счётчика магазина; добавлен `test_scraper_completeness.py` (10 проверок).
  - У MasterOK много товаров «по запросу» (без цены). Страница из одних таких считалась ошибкой «пустая выдача» — теперь это честное «конец не подтверждён», без ложной деградации магазина. Пауза между страницами поднята 0.4 → 1.2 с: при быстром листании сайт отдаёт страницы без товаров.
- ⚠️ **НЕ ЗАКОММИЧЕНО, ждёт выхода «Детского мира»: новые `MASTEROK_CATEGORIES` в `config.py`.** По решению владельца («все надо») категории переписаны с подразделов «Распродажи витрины» на настоящий каталог. Правка лежит в рабочем дереве и не закоммичена, потому что `config.py` одновременно правит Antigravity под «Детский мир»; коммитить его незавершённую работу нельзя (AGENTS.md). **Кто будет коммитить `config.py` — пусть включит и эту правку.**
  - Проверено вживую после правки адаптера: **1199 товаров вместо 99**, ни одной категории с ошибкой (6 подтверждают полноту, 4 честно неполные из-за товаров без цены), полный обход 350 с.
  - Важное про сайт: у masterok.kz **нет** общих разделов «Инструменты», «Сварочное», «Садовое», «Строительное» — такими именами названы только подразделы распродажи и уценки. Каталог устроен по видам техники: Запчасти, Складское, Минитракторы, Мотоблоки, Деревообработка, Техника для уборки, Генераторы.
  - Родительские разделы выводят только свои товары, а счётчик считает и дочерние (Складское: 690 заявлено, 30 своих). Дочерние разделы почти целиком «цена по запросу», поэтому подключать их отдельно смысла нет — проверено.
