...
- Дата: 2026-09-20 13:50, Asia/Almaty.
- Последнее действие: Claude исправил F01–F04 по аудиту P06 (ветка `dev/p06-search-analytics`, коммиты
  `032be70` — отчёт Codex, `d259374` — исправления). **Нужен повторный аудит; push не выполнялся.**
- Что изменено:
  - F01 — город приводится к справочнику до любой записи (известный → название, «все»/«Казахстан» → общие
    значения, прочее → «Неизвестно»); источник проверяется по фиксированному набору ("catalog", "live").
  - F02 — названные в запросе признаки модели (номер, объём памяти, слова pro/max/plus…) обязаны совпасть,
    совпадение слов — по границам слова: RTX 4060 больше не отвечает на «RTX 5090», iPhone 15 — на «15 Pro 256GB».
  - F03 — при сбое всех опрошенных источников исход ERROR, а не NOT_FOUND; сбойная пустая выдача не кэшируется.
  - F04 — единица учёта = одно обращение человека (`user_search=True` в /api/best-price); вложенное живое
    обновление и фоновые обходы сохраняют телеметрию, но не копят спрос и не приближают порог третьего повтора.
  - Уточнён комментарий о хеше ключа: он избавляет от хранения текста, но не защищает от словарного подбора.
- Тесты: Python **476 OK** (test_search_analytics.py: 44, из них 7 сквозных на F01–F04 через настоящие обёртки
  поиска без сети), frontend **8/8 PASS**.
- Правило владельца: **без аудита ничего не пушим**. На проде v5.10.0; код прода не менялся.
- Пишущих исполнителей нет: Claude закончил. Следующий — Codex: повторный аудит P06.
- Команда для Codex: «Прочитай AGENTS.md, docs/DEVELOPMENT_PLAN.md и docs/AGENT_HANDOFF.md. Проведи повторный
  аудит P06 на ветке dev/p06-search-analytics, не меняя код (diff b1c6f2a..d259374). Проверь приёмку F01–F04:
  произвольный city через реальный API не попадает ни в одну таблицу; неверная модель/память/Pro-вариант → weak,
  точная модель → found, generic и аксессуарные запросы работают; таймаут всех источников → error, успешная
  пустая выдача → not_found, сбой не кэшируется; два live-поиска → total=2 без текста, третий → текст появляется,
  фоновые обходы спрос не копят. Запиши выводы в docs/P06_AUDIT_CODEX.md, обнови карточку и реестр.»
...
| P06 | IN_PROGRESS | Claude | Codex: F01–F04 требуют исправлений | Не выпущен |
...
Статус: IN_PROGRESS — аудит Codex завершён; исправить F01–F04 по docs/P06_AUDIT_CODEX.md.
