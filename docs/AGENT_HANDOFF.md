...
- Дата: 2026-09-20 13:22, Asia/Almaty.
- Последнее действие: **выпущена v5.10.0** (P03+P04+P05) по поручению владельца «Выпускай». Прод отвечает 5.10.0.
- Ход выпуска: push main + тег → сборка образа **упала** на frontend-тесте P04 (`'11,732' !== '11 732'`): страница
  форматировала числа по локали браузера, а сборочный агент работает в английской локали. Исправлено коммитом
  `8478f91` — счётчики выводятся в формате ru-RU, как и цены; тест P04 теперь открывает страницу с английской
  локалью и ловит расхождение вне русской машины. Образ с прода до исправления не публиковался, прод оставался 5.9.0.
- Тег `v5.10.0` по решению владельца переставлен на выкаченный коммит `8478f91` (у прежнего образа не было).
  Сборки main и тега — зелёные.
- Проверка прода чтением (13:16–13:20): `/api/version` → 5.10.0; `/api/stats?city=Астана` — 78 632 товара,
  12 912 предложений, 194 арбитраж; вкладки витрины совпадают с разбивкой (all 12 912 / super 991 / arbitrage 194);
  `/monitoring` и `/static/js/format.js` — 200.
- Не проверено на проде: запись телеметрии изнутри, реальные обходы, AI и Telegram, Docker-образ вручную.
  Админские страницы под учётной записью владельца не открывались.
- Пишущих исполнителей нет. Следующий шаг — по решению владельца: этап P06 (аналитика поиска) или другая задача.
- Напоминание: правило «без аудита не пушим» продолжает действовать для следующих этапов.
