...
- Дата: 2026-09-19 19:31, Asia/Almaty — повторный аудит B01/B02 завершён.
- Последнее действие: Codex проверил исправления `158c68e` на HEAD `d0790b4`: **B01/B02 закрыты**, A01–A05 остаются закрытыми. **P01 — READY по аудиту кода**, не внедрён и не проверен на проде. [Актуальный отчёт](P01_AUDIT_CODEX.md).
- Ветка `dev/p00-baseline`, база P01 `12672ec`. Версия приложения `5.7.1`, `schema_version = 5` (не повышались). Push/выпуска не было.
- Полный suite независимо повторён Codex с временным `DATA_DIR`: 339 тестов OK, 32.027 с. Дополнительные воспроизведения B01/B02 проходят. Прод не проверялся.
...
| P01 | READY (аудит кода) | Antigravity → Claude; подготовка выпуска — Claude (с 19:34) | Codex: A01–A05 и B01/B02 закрыты | Не проверен; выпуск не выполнялся |
...
## R-P01 — подготовка выпуска P01 (Claude, IN_PROGRESS)

- Поручение владельца 2026-09-19 ~19:34: «Подготовить выпуск P01» — слить изоляцию тестов (ветка
  claude/affectionate-lehmann-490dc0), повысить версию до 5.8.0, CHANGELOG/дорожная карта, локальный smoke сервера на
  копии БД. Push в main, тег и выкат — только после отдельного разрешения владельца.
- Исполнитель: Claude, начало 19:34 Asia/Almaty. Файлы: version.py, CHANGELOG.md, ROADMAP_AND_LOG.md, docs, файлы ветки изоляции.
