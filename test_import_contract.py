"""Модули должны импортироваться и на Python младше локального.

Зачем отдельная проверка: локально проект живёт на 3.14, где аннотации вычисляются лениво
(PEP 649), а CI и образ — на 3.12, где аннотация модульного уровня вычисляется прямо при импорте.
Из-за этого забытый импорт из `typing` выглядит локально безупречно, а на проде роняет приложение
...
class AnnotationImportTest(unittest.TestCase):
    def test_annotations_evaluated_at_import_have_their_names_imported(self):
        ...
        self.assertEqual(sorted(set(missing)), [],
                         "имя из аннотации не объявлено в модуле — на Python 3.12 это NameError "
