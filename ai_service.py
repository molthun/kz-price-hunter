...
async def parse_natural_query(query: str, current_city: str = "Все", force: bool = False,
    ...
    prompt = f"""
Ты — высокоточный аналитик поисковых запросов в казахстанском агрегаторе цен (Kaspi, DNS, Белый Ветер, Лемана ПРО, Arbuz, 12 Месяцев, Мечта, Технодом и др.).
...
async def ask_ai_consultant(
    ...
    1. Распознает потребность пользователя и ищет в локальной базе магазинов лучшие товары.
    ...
    prompt = f"""
Ты — персональный AI-консультант сервиса KZ Price Hunter (Казахстан).
Твоя цель — помочь пользователю выбрать оптимальный товар, сравнить варианты, указать на подводные камни и посоветовать, где купить выгоднее всего.

АКТУАЛЬНЫЕ ДАННЫЕ ИЗ МАГАЗИНОВ КАЗАХСТАНА (Kaspi, DNS, Белый Ветер, Лемана ПРО, Arbuz, 12 Месяцев, Мечта, Sulpak и др.):
{catalog_context}
...
"""
    ...
    answer_fallback = "Я проанализировал ваш запрос по каталогам магазинов Казахстана.\n\n"
    if top_items:
        best = top_items[0]
...
async def normalize_product_titles_batch(
    ...
    *,
    ...
    system_prompt = (
        "Ты эксперт по каталогам товаров, техники и потребительских цен в Казахстане. "
